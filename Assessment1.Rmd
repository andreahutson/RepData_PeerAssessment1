...
# You can also do this without even running the intervalSteps function, with a built in function
# from q plot
...
```{r echo=TRUE}
# calculate the sum of the inverse of complete cases of activity (where "complete.cases" is FALSE)
...
# Label the day of the week as a weekend or a weekday
    weekdays <- vector()
    for (day in daysofweek) {  # for each day in the list
        if (day == "Saturday" | day == "Sunday") { # if the day is Sat or Sun
...
# Merge with the activity data
    daysOfWeek <- as.data.frame(cbind(days, daysofweek, weekdays), 
...
qplot(interval, steps, data=all, geom = "line",
      facets = weekday~. , stat="summary", fun.y = "mean")
```
